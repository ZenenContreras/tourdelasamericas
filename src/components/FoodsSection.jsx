...
import { Utensils, ArrowRight, ChevronRight } from 'lucide-react';
...
const FoodsSection = () => {
  ...
  const handleRegionClick = (regionId) => {
    ...
    setTimeout(() => setClickedRegion(null), 2000);
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center justify-center mb-4 sm:mb-6">
            <Utensils className="h-6 w-6 sm:h-8 sm:w-8 text-emerald-600 mr-2 sm:mr-3" />
            ...
            >
              <div className="absolute inset-0 w-full h-full overflow-hidden">
                ...
                <div className="absolute inset-0 flex flex-col justify-center p-6 sm:p-8">
                  ...
                  <p className="hidden sm:block text-sm sm:text-base text-white mb-4 sm:mb-6 max-w-2xl drop-shadow-lg" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.7)' }}>
                    {region.description}
                  </p>
                  
                  <div className="flex items-center text-white mt-auto bg-black/15 px-3 py-2 rounded-lg inline-flex">
                    <span className="text-sm sm:text-base font-medium drop-shadow-md" style={{ textShadow: '0 1px 2px rgba(0,0,0,0.5)' }}>{t('foodSection.viewFood')}</span>
                    <motion.div 
                      className="ml-2"
                      initial={{ x: 0 }}
                      whileHover={{ x: 5 }}
              ...
              <AnimatePresence>
                {clickedRegion === region.id && (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="absolute inset-0 bg-emerald-600/90 backdrop-blur-sm flex items-center justify-center"
                  >
                    <motion.div
                    ...
                    >
                      <span className="text-white text-xl font-bold block mb-2">{t('comingSoon')}</span>
        ...
        >
          <motion.button
            className="inline-flex items-center bg-emerald-600 text-white px-5 py-2.5 sm:px-6 sm:py-3 rounded-lg text-base sm:text-lg font-medium hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-500/30"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            onClick={() => window.location.href = '/catalogo/comidas'}
          >
