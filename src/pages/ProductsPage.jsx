          ...
          <motion.button
            ...
            className={`flex items-center space-x-2 px-4 py-2.5 rounded-lg shadow-sm transition-all duration-300 ${
              showFilters 
                ...
                : 'bg-white/80 backdrop-blur-sm text-indigo-600 hover:bg-white border border-indigo-100 hover:border-indigo-300'
            }`}
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
          >
            <Filter className={`h-5 w-5 ${showFilters ? 'text-white' : 'text-indigo-600'}`} />
        ...
        <AnimatePresence>
          {showFilters && (
            <motion.div
              ...
              className="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg p-6 mb-8 overflow-hidden border border-indigo-100"
            >
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-3 sm:gap-4">
            {products.map((item) => (
              <motion.div
                ...
                exit={{ opacity: 0, y: 20 }}
                className="bg-white/80 backdrop-blur-sm rounded-xl shadow-sm overflow-hidden group hover:shadow-md transition-all duration-300 border border-indigo-50 hover:border-indigo-200"
              >
                <div className="relative">
                  {item.imagen_url ? (
                    <div className="aspect-w-1 aspect-h-1 w-full overflow-hidden bg-gradient-to-br from-indigo-50 to-blue-50">
                      <img
                        src={item.imagen_url}
                        alt={item.nombre}
                    ...
                    <div className="aspect-w-1 aspect-h-1 w-full bg-gradient-to-br from-indigo-100 to-blue-50 flex items-center justify-center">
                  ...
                  <div className="flex flex-col h-full">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-2 border-t border-gray-100">
                      ...
                      {isInCart(item.id) ? (
                        ...
                        <button
                          onClick={() => handleAddToCart(item)}
                          className="flex items-center space-x-1 bg-indigo-600 text-white px-2 py-1 rounded-lg hover:bg-indigo-700 transition-all duration-300 text-xs font-medium disabled:bg-gray-400 disabled:cursor-not-allowed"
                          disabled={item.stock === 0}
                        >
                          <ShoppingCart className="h-3 w-3" />
                          <span>{item.stock === 0 ? t('common.outOfStock') : t('common.add')}</span>
                        </button>
                      )}
                    </div>
                  </div>
